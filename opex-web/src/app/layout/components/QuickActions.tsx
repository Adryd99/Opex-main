  ...
  return (
    <div className="relative" ref={quickActionsRef}>
      <button
        ...
        className={`${compact ? 'w-8 h-8 rounded-lg' : 'p-3 rounded-2xl'} bg-opex-dark text-white shadow-lg hover:scale-105 active:scale-95 transition-all flex items-center justify-center dark:bg-opex-teal dark:text-slate-950 ${showQuickActions ? 'rotate-45 bg-slate-700 dark:bg-teal-300' : ''}`}
      >
        <Plus size={compact ? 18 : 24} />
      </button>

      {showQuickActions && (
        <div className="absolute right-0 mt-3 w-64 bg-app-surface rounded-2xl shadow-2xl border border-app-border py-2 z-50 animate-in fade-in zoom-in-95 duration-200">
          <div className="px-4 py-2 border-b border-app-border mb-1">
            <p className="text-[10px] font-bold text-app-tertiary uppercase tracking-widest">{t('quickActions.title')}</p>
          </div>
